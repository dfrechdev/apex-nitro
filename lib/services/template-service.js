...
const installCssPreprocessors = async (cssExtensions, installPath) => {
	if (cssExtensions.includes('.less')) {
		await npmService.installPackage('less', installPath);
	}

	if (cssExtensions.includes('.styl')) {
		await npmService.installPackage('stylus', installPath);
	}

	if (cssExtensions.includes('.sass') || cssExtensions.includes('.scss')) {
		await npmService.installPackage('node-sass', installPath);
	}
};
